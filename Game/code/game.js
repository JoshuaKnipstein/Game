  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "lava";
	  //Adds teleport function to green blocks.
	  else if (ch == "y")
...
// Actor class of Coin. Adds size and movement.
function Coin(pos) {
...
// Adds the Jump Actor as a class. Blue Cube. 
function Jump(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
...
// Adds the Slow Actor as a class. Purple Cube.
function Slow(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.9, 0.9);
  // Make it go back and forth in a sine wave.
...
//Adds Movement to the Jump and Slow Powerups. 
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (obstacle == "teleporter"){
	  this.pos = new Vector(4,14); //Sends Player Back To Start.
	  }
  else if (obstacle)
...
//Gives directions according to what Actor class item the player touched. And sets up Win Condition.
